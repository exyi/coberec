        ...
        public static IMethod ImplementValidateIfNeeded(this VirtualType type, EmitContext cx, TypeDef typeSchema, TypeSymbolNameMapping typeMapping, IEnumerable<ValidatorUsage> validators)
        {
            ...
            foreach(var v in Enumerable.Concat(GetImplicitNullValidators(type, typeSchema, typeMapping), validators))
            {
                ...
                Func<IL.ILVariable, IL.ILInstruction> makeField(string[] field, IType expectedType) =>
                    @this => {
                        if (field.Length > 1)
                            throw new NotSupportedException($"Field paths like '{string.Join(".", field)}' are not supported.");
                        ...
                        if (!conversion.IsValid)
                            throw new Exception($"Validator '{v.Name}' of type '{expectedType}' can not be applied on field of type '{x.resultType}'");
