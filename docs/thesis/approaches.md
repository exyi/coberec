...
[example JSON files](https://fsharp.github.io/FSharp.Data/library/JsonProvider.html),
[OpenAPI schema](https://github.com/fsprojects/OpenAPITypeProvider),
[database schema](https://fsprojects.github.io/SQLProvider/)
and even [result schema of SQL queries](https://github.com/demetrixbio/FSharp.Data.Npgsql).
...
For example, ScalaTest, a common test library, contains an [`assert` macro](https://www.scalatest.org/user_guide/using_assertions) that looks like the plain old `assert` function.
...
Both of these libraries reduce the boilerplate code, while Chimney does not have a runtime cost and mapping errors will arise during compilation, not at runtime.
...
In Javascript, the `{}` expression creates an empty object and simply assigning to `obj.myField` creates a new field if it does not exist already.
